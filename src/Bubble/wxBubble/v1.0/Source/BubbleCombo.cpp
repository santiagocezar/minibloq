...
// TODO: Make the image pointer constant?
void BubbleCombo::append(const wxString& itemCaption, wxImage *icon, int iconW, int iconH)
{
    ...
    long id = wxNewId();
    wxMenuItem *newItem = new wxMenuItem(popList, id, itemCaption);

    if (newItem == nullptr)
        return;

    if (icon) {
        // Usar el tamaño original del ícono si no se definió ningún otro.
        if (iconW > 0 && iconH > 0)
    ...
    // Si está ordenado el menú, averiguar donde tendría que ir el nuevo item
    // BUG: Probarlo con el ejemplo de PID, porque el ordenamiento falla de un modo extraño...
    // BUG: ^ No sé de que habla
    int insertAt = -1;

    if (getSorted()) {
        // TODO: Ordenamiento natural
        for (int i = 0; i < count; i++) {
            wxMenuItem *item = popList->FindItemByPosition(i);

            if (itemCaption < item->GetItemLabel()) {
                insertAt = i;
                break;
            }
        }
    ...
    if (insertAt >= 0) popList->Insert(insertAt, newItem);
    else popList->Append(newItem);
